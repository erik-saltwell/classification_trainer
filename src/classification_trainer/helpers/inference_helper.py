...
def _build_sampling_params(
    ...
    stop_token_ids = template.get_eos_token_ids(tokenizer)

    kwargs: dict[str, Any] = dict(
        temperature=inference_info.temperature,
        top_p=inference_info.top_p,
        max_tokens=inference_info.max_new_tokens,
        stop=list(template.stop_strings) if template.stop_strings else None,
        stop_token_ids=stop_token_ids if stop_token_ids else None,
    )

    if inference_info.repetition_penalty is not None:
        kwargs["repetition_penalty"] = inference_info.repetition_penalty

    return SamplingParams(**kwargs)


def generate_label_text_with_vllm(
    llm: "LLM",
    prompt_text: str,
    inference_info: InferenceInfo,
    template: ChatTemplateInfo,
    ...
    sampling_params = _build_sampling_params(inference_info, template, tokenizer)
    outputs = llm.generate([prompt_text], sampling_params, lora_request=lora_request)
    return template.trim_at_stop_strings(outputs[0].outputs[0].text)


def generate_label_texts_with_vllm(
    llm: "LLM",
    prompt_texts: list[str],
    inference_info: InferenceInfo,
    template: ChatTemplateInfo,
    ...
    """
    Batched inference via vLLM — submits all prompts in a single generate() call.

    Unlike the HF version (which loops one-at-a-time due to Unsloth bugs),
    vLLM handles batching internally with continuous batching for optimal throughput.
    """
    cleaned = [_clean_prompt_ending(p, template) for p in prompt_texts]
    tokenizer = llm.get_tokenizer()
    sampling_params = _build_sampling_params(inference_info, template, tokenizer)
    outputs = llm.generate(cleaned, sampling_params, lora_request=lora_request)
    return [template.trim_at_stop_strings(o.outputs[0].text) for o in outputs]


def add_inferred_column_with_vllm(
    dataset: Dataset,
    dataset_info: DatasetInfo,
    llm: "LLM",
    inference_info: InferenceInfo,
    ...
    if output_column_name in dataset.column_names:
        dataset = dataset.remove_columns(output_column_name)

    prompt_texts: list[str] = dataset[prompt_column_name]

    if logger is not None:
        with logger.status("vLLM batch inference"):
            predictions = generate_label_texts_with_vllm(llm, prompt_texts, inference_info, template, lora_request)
    else:
        predictions = generate_label_texts_with_vllm(llm, prompt_texts, inference_info, template, lora_request)

    return dataset.add_column(output_column_name, predictions)  # type: ignore[return-value]


def cleanup_vllm(llm: "LLM") -> None:
    """Free GPU memory held by a vLLM LLM instance."""
    import gc
